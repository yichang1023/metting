...
function validateGeminiUploadUrl(rawUrl) {
  let parsed;
  try {
    parsed = new URL(String(rawUrl || ''));
  } catch {
    throw new HttpError(400, 'INVALID_UPLOAD_URL', 'Gemini 上傳工作網址格式不正確。');
  }

  if (parsed.protocol !== 'https:'
      || parsed.hostname !== 'generativelanguage.googleapis.com'
      || !parsed.pathname.startsWith('/upload/')) {
    throw new HttpError(400, 'INVALID_UPLOAD_URL', '拒絕非 Gemini Files API 的上傳網址。');
  }
...
export async function uploadChunkToSession({
  ...
  const safeUploadUrl = validateGeminiUploadUrl(uploadUrl);
  const parsedOffset = Number(offset);
  const parsedTotal = Number(totalSize);

  if (!Number.isSafeInteger(parsedOffset) || parsedOffset < 0) {
    throw new HttpError(400, 'INVALID_UPLOAD_OFFSET', '音檔分段上傳位置不正確。');
  }
  if (!Number.isSafeInteger(parsedTotal) || parsedTotal <= 0) {
    throw new HttpError(400, 'INVALID_TOTAL_SIZE', '音檔總大小不正確。');
  }
  if (typeof dataBase64 !== 'string' || !dataBase64) {
    throw new HttpError(400, 'EMPTY_UPLOAD_CHUNK', '沒有收到音檔分段內容。');
  }

  const maxChunkBytes = clampInteger(
    process.env.MAX_PROXY_CHUNK_BYTES,
    2 * 1024 * 1024,
    256 * 1024,
    3 * 1024 * 1024
  );

  // Base64 只允許標準字元與尾端 padding，避免把任意大型文字當成音檔處理。
  if (!/^[A-Za-z0-9+/]+={0,2}$/.test(dataBase64)) {
    throw new HttpError(400, 'INVALID_CHUNK_ENCODING', '音檔分段不是有效的 Base64。');
  }

  const chunk = Buffer.from(dataBase64, 'base64');
  if (!chunk.length) {
    throw new HttpError(400, 'EMPTY_UPLOAD_CHUNK', '音檔分段解碼後是空的。');
  }
  if (chunk.length > maxChunkBytes) {
    throw new HttpError(413, 'UPLOAD_CHUNK_TOO_LARGE', `單一音檔分段不得超過 ${(maxChunkBytes / 1048576).toFixed(1)} MB。`);
  }
  if (parsedOffset + chunk.length > parsedTotal) {
    throw new HttpError(400, 'UPLOAD_RANGE_INVALID', '音檔分段超出檔案總大小。');
  }

  const finalChunk = Boolean(isFinal);
  if (finalChunk && parsedOffset + chunk.length !== parsedTotal) {
    throw new HttpError(400, 'FINAL_CHUNK_MISMATCH', '最後一段音檔大小與總大小不一致。');
  }
  if (!finalChunk && parsedOffset + chunk.length >= parsedTotal) {
    throw new HttpError(400, 'FINAL_CHUNK_REQUIRED', '最後一段音檔必須標記為完成。');
  }
  ...
  const response = await fetchWithTimeout(
    safeUploadUrl,
    {
      ...
      headers: {
        ...
        'X-Goog-Upload-Offset': String(parsedOffset),
        'X-Goog-Upload-Command': finalChunk ? 'upload, finalize' : 'upload'
      },
      body: chunk
    },
    clampInteger(process.env.GEMINI_UPLOAD_CHUNK_TIMEOUT_MS, 55_000, 10_000, 60_000)
  );
  ...
  if (!response.ok) {
    ...
    throw new HttpError(
      response.status,
      'GEMINI_UPLOAD_CHUNK_FAILED',
      `Gemini 接收音檔分段失敗：${detail.message}`
    );
  }
  ...
  if (!finalChunk) {
    return {
      ok: true,
      nextOffset: parsedOffset + chunk.length,
  ...
  if (!file.name) {
    throw new HttpError(502, 'UPLOAD_FINAL_RESPONSE_INVALID', 'Gemini 完成上傳後沒有回傳檔案資訊。');
  }
