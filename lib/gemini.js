...
export async function createUploadSession({ mimeType, size, displayName }) {
  ...
  for (const { slot, key } of slots) {
    ...
    if (response.ok) {
      ...
      const headerGranularity = Number(response.headers.get('x-goog-upload-chunk-granularity'));
      const chunkGranularity = Number.isSafeInteger(headerGranularity) && headerGranularity > 0
        ? headerGranularity
        : 8 * 1024 * 1024;
      return { uploadUrl, keySlot: slot, chunkGranularity };
    }
...
export async function uploadBinaryChunkToSession({
  uploadUrl,
  offset,
  totalSize,
  chunk,
  ...
  const response = await fetchWithTimeout(
    ...
    clampInteger(process.env.GEMINI_UPLOAD_CHUNK_TIMEOUT_MS, 90_000, 10_000, 240_000)
  );
